...
class NAuthGate extends StatefulWidget {
  const NAuthGate({super.key});
  @override State<NAuthGate> createState() => _NAuthGateState();
}

class _NAuthGateState extends State<NAuthGate> {
  @override
  void initState() {
    super.initState();
    unawaited(data.initialize());
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        if (data.loggedIn && data.userId != null) return const NHome();
...
class _ShortVideoCardState extends State<ShortVideoCard> {
        ...
        builder: (sheetContext) {
          return StatefulBuilder(
            builder: (context, setSheetState) {
              if (loadingComments) {
                unawaited(data.loadComments(widget.post.id).then((rows) {
                  if (sheetContext.mounted) setSheetState(() { comments = rows; loadingComments = false; });
                }));
              }
              return SafeArea(
                child: Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: Column(children: [
                      Text('التعليقات (${widget.post.comments})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 10),
                      Expanded(child: loadingComments
                        ? const Center(child: CircularProgressIndicator())
                        : comments.isEmpty
                          ? const Center(child: Text('لا توجد تعليقات بعد'))
                          : ListView.builder(itemCount: comments.length, itemBuilder: (_, i) {
                              final c = comments[i]; final profile = c['profile'] is Map ? Map<String,dynamic>.from(c['profile']) : <String,dynamic>{};
                              final mine = c['user_id']?.toString() == data.userId;
                              return ListTile(
                      ...
                      Row(children: [
                        Expanded(child: TextField(controller: controller, maxLength: 1000, decoration: const InputDecoration(hintText: 'اكتب تعليقك...'))),
                        const SizedBox(width: 8),
                        IconButton.filled(onPressed: () async {
                          final ok = await data.addComment(widget.post, controller.text);
                          if (ok && sheetContext.mounted) { final rows = await data.loadComments(widget.post.id); setSheetState(() { comments = rows; controller.clear(); }); }
  ...
  Future<void> _share() async {
    ...
    unawaited(data.registerShare(widget.post));
  }
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _openEditProfile(BuildContext context) async {
    ...
    final bio = TextEditingController(text: (data.currentBio ?? ''));
...
class _ProfileVideoTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onLongPress: () async {
        if (post.username != data.username) return;
        final action = await showModalBottomSheet<String>(
          context: context,
          builder: (context) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ListTile(leading: Icon(post.isPinned ? Icons.push_pin : Icons.push_pin_outlined), title: Text(post.isPinned ? 'إلغاء تثبيت الفيديو' : 'تثبيت الفيديو'), onTap: () => Navigator.pop(context, 'pin')),
            ListTile(leading: const Icon(Icons.delete_outline), title: const Text('حذف الفيديو'), onTap: () => Navigator.pop(context, 'delete')),
          ])),
        );
        if (action == 'pin') await data.pinPost(post);
        if (action == 'delete') await data.deletePost(post);
      },
      child: Stack(
        fit: StackFit.expand,
        children: [
          ...
          if (post.isPinned)
            const Positioned(top: 7, right: 7, child: Icon(Icons.push_pin_rounded, size: 19)),
          Positioned(
            bottom: 6,
            right: 6,
            child: Row(
...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  void initState() {
    super.initState();
    unawaited(_registerVisit());
  }

  Future<void> _registerVisit() async {
    try {
      final row = await data.supabase.from('public_profiles').select('id').eq('username', widget.username).maybeSingle();
      final id = row?['id']?.toString();
      if (id != null) await data.registerProfileVisit(id);
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: data,
      builder: (_, __) {
        final userPosts = data.postsOf(widget.username);
        final isFollowing =
            data.following.contains(widget.username);
